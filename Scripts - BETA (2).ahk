...
Gui, 1:Add, Text, x10 y50 , Includes: Calendar Extender, Attorney Finder,`n`t`t  Remarks Suggester, and Record Opener 
...
ExampleQuickDate1:
Run H:\Docketing\AutoHotKey\.ini Files - DO NOT TOUCH!\ImageSearch\QuickDate1.png   ; Probably update to ActiveX
return

ExampleQuickDate2:
Run H:\Docketing\AutoHotKey\.ini Files - DO NOT TOUCH!\ImageSearch\QuickDate2.png	; Probably update to ActiveX	 
return
...
OpenHelp:
		...
		Gui, 13:Add, Button, gExampleQuickDate1, Example 1
		Gui, 13:Add, Button, gExampleQuickDate2, Example 2  ; Probably update to ActiveX
		...
		Gui, 13: Tab, 6
		Gui, 13:Font, bold underline
		Gui, 13:Add, Text, x10 y80 w390 +Center, Calendar Extender/Attorney Finder/`nRemarks Suggester/Record Opener
		Gui, 13:Font
		Gui, 13:Add, Text, x10 y120 w390 +Center +Wrap, Default Keybinding (Ctrl + Right Click)`n`nCalendar Extender allows you to quickly extend a date in docketing by certain intervals of time (1W, 2W, 1M, 2M, 3M, 6M, 1Y). If auto-remarks are turned on, remarks will be automatically added, noting the date, the duration of the extension, and your username. Only the Responsible person's initials need to be added.`n`nAttorney Finder allows you to select a block of attorneys in docketing, which will then remove the duplicates, substitute irregular names, and add your AT partner to your Clipboard. You can then paste into the CC field and move the responsible person to the To field. No more 2JCS/2JSC issues!`n`nRemarks Suggester is functional but barebones. Selecting an action will provide typical remarks to paste. Currently works on Status Checks.`n`nRecord Opener will open a Client Code directly into docketing. You can customize which menu options appear on the pop-up, or force Patent/TM only. Works anywhere you can select text and is VERY quick.`n`nQuick Record Opener ignores your other settings and is always functional. You can customize which button opens Patent and which opens Trademark. Activate with Alt + RightClick/LeftClick
		...
		Gui, 13:Font
		Gui, 13:Add, Text, x10 y120 w390 +Center +Wrap, Default Keybinding (Automatic)`n`nThe Custom SideBar places a very thin (3 pixels wide!) persistent bar on the right side of your right monitor. You can also customize the position by adjusting the coordinate field.`n`n<Add a picture here!>`n`nAn example SideBar will show when adjusting the values so you can place it as desired. If positioned correctly, you can barely see it when you have other windows up.`n`nYou can customize this bar to open a website or program, and many AutoHotkey functions are available as well.`n`nDefault options include Calculator, Internet Explorer, Kronos, and Google. You could save WIPO or EPO websites if you'd like.`n`nOnly a single custom option can be saved at this time, but perhaps additional sections could be added.
...
IniWrite, v1.16.2017, H:\Docketing\AutoHotKey\.ini Files - DO NOT TOUCH!\ImageSearch\%A_Username%.ini, Version, Version
Gui, 8:Destroy
Gui, 9:Destroy
Gui, 10:Destroy
Gui, 11:Destroy
