...
Gui, Font, s12
Gui, Add, Text, x10 w255 Section Center, Select the Corres Log Date`n(Hold Shift to select multiple days.)
...
Gui, Add, DropDownList, Center xp yp+50 wp vSendOnBehalfOf, *INTLClerksDocketing||

Gui, Font,
Gui, Add, ListView, Grid -Multi r16 w650 xs+265 ys, Correspondence (Document Description)|Document #|Client.Matter|Agent|AT Partner|Agent Email
Gui, Font, bold
Gui, Add, Text, Center xp+655 yp w150,  ;Use Control + F12 to clean data from Facing query. You can then paste into Excel template.
Gui, Font,
Gui, Font, s12
Gui, Add, Button, gGrabFacingInfo  xp yp w150 h45, Pull Corres Log Data from Excel
Gui, Add, Button, gClearFacingInfo xp yp+50 wp h35, Clear Grabbed Info
Gui, Add, Button, gShowSignature   xp yp+110 wp hp, Saved Signature
Gui, Add, Button, gCreateEmails    xp yp+105 wp hp, Create Emails
...
Gui, Signature: Add, Picture, ,H:\Docketing\AutoHotKey\.ini Files - DO NOT TOUCH!\ImageSearch\HelpImages\SaveSignature.PNG
...
SplashtextOn, 500, 0, Select your Corres Log information. I'm going to grab it from Excel in 5 seconds.
Sleep 1000
SplashtextOn, 500, 0, Select your Corres Log information. I'm going to grab it from Excel in 4 seconds.
Sleep 1000
SplashtextOn, 500, 0, Select your Corres Log information. I'm going to grab it from Excel in 3 seconds.
Sleep 1000
SplashtextOn, 500, 0, Select your Corres Log information. I'm going to grab it from Excel in 2 seconds.
Sleep 1000
SplashtextOn, 500, 0, Select your Corres Log information. I'm going to grab it from Excel in 1 second.
Sleep 1000
SplashTextOff
xl.Selection.Copy
...
LV_Add("", FaceAttyParts1, FaceAttyParts2, FaceAttyParts3, FaceAttyParts4, FaceAttyParts5, FaceAttyParts4)
}
LV_ModifyCol()
LV_ModifyCol(1, "Logical")
LV_ModifyCol(4, "SortAsc")
...
Loop %FixExtra%
{
LV_GetText(Attorneys, A_Index, 4 )   ;[, ColumnNumber]) ¶
...
StringSplit, SortedAttys, SortedAttys, `n
	if SortedAttys0 = 0
		{
		MsgBox, , You didn't add any information!, Add some Corres Log information and try again.
		Gui, Show
		return
		}
...
EmailBodyContents := "<HTML><style> table {border-style: solid; border-collapse:collapse; border-width: 1px;} td {border-style: solid; border-collapse:collapse; border-width: 1px; text-align: center; vertical-align: middle; padding: 5px;} p{color: blue;} </style><p><BODY><p>Hello Agents:<br><br>Here is a list of correspondence which we received on " Today ". The documents for each matter listed have been saved to iManage so that you can simply retrieve the documents from iManage when you are ready to docket them.<br><br>Urgent mail is bold and red. Please process these items first.</p><br><table><tr><td align=center valign=middle>" DueDate "</td><td align=center valign=middle>" ActionType "</td><td align=center valign=middle><a href=""iwl:dms=OC-IDMS&&lib=KMOB&&num=" ActionDue "&&ver=1&&latest=1"">" ActionDue "</a></td><td align=center valign=middle>" Indicator "</td><td align=center valign=middle>" ClientCode "</td><td align=center valign=middle>" Responsible "</td></tr>"
MailItem.To := EmailAddress 
EmailSubject := Responsible " | " DateRange " International Correspondence Log"
...
LV_GetText(Responsible, A_Index, 6 )   ;[, ColumnNumber]) ¶
LV_GetText(EmailAddress, A_Index, 7 )   ;[, ColumnNumber]) ¶
EmailBodyContents := EmailBodyContents "<tr><td align=center valign=middle>" DueDate "</a></td><td align=center valign=middle>" ActionType "</td><td align=center valign=middle><a href=""iwl:dms=OC-IDMS&&lib=KMOB&&num=" ActionDue "&&ver=1&&latest=1"">" ActionDue "</a></td><td align=center valign=middle>" Indicator "</td><td align=center valign=middle>" ClientCode "</td><td align=center valign=middle>" Responsible "</td></tr>"
return
...
IfEqual, OutputVarControl, Button1
	Help := "This grabs your Corres Log information directly from Excel.`n`nAfter clicking, you'll have five seconds to highlight your data in Excel.`nYou can also select your information prior to clicking the button.`n`nIf you've already grabbed some information,`nthis will append your new Corres Log information to the list."
	...
	Help := "This will sort your data by ""Agent"" and determine how many emails to create.`n`nYou'll have a chance to cancel if something doesn't look right, so don't worry."
