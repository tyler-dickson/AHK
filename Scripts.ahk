...
The New CPI Update... Yay. - 10/11/2021
The Seriously Get Ready for New CPI Update - 9/20/2021
The Get Ready for New CPI Update - 6/21/2021
Miscellaneous Updates Update - 4/2/2021
...
global ScreenWidthPlusFifteenPercent   := ScreenWidth + (ScreenWidth * .15) 
global ThirdScreenHeight    := ScreenHeight * .33                           
global HalfScreenHeight     := ScreenHeight * .5                            
global TwoThirdScreenHeight := ScreenHeight * .67		                    
	
global ScreenWidthPointFive        := Screenwidth * .005                    
global ScreenWidthOnePercent       := Screenwidth * .01                     
global ScreenWidthTwoPercent       := Screenwidth * .02                     
global ScreenWidthThreePercent     := ScreenWidth * .0333	                
global ScreenWidthFourPercent      := ScreenWidth * .04                     
global ScreenWidthFivePercent      := ScreenWidth * .05                     
global ScreenWidthFifteenPercent   := ScreenWidth * .15                     
global ScreenWidthSixteenPercent   := ScreenWidth * .16                     
global ScreenWidthMinusFivePercent := 0 - ScreenWidthFivePercent            
...
IniRead, ShowSuperEditOptions, C:\Users\%Computername%\AHKPrefs.ini, Username, ShowSuperEditOptions
	if ShowSuperEditOptions = ERROR	
		{
		ShowSuperEditOptions = 0
		IniWrite, %ShowSuperEditOptions%, C:\Users\%Computername%\AHKPrefs.ini, Username, ShowSuperEditOptions
		IniRead,   ShowSuperEditOptions,  C:\Users\%Computername%\AHKPrefs.ini, Username, ShowSuperEditOptions
		}	

IniRead, ShowAttyFinderButton, C:\Users\%Computername%\AHKPrefs.ini, Username, ShowAttyFinderButton
	if ShowAttyFinderButton = ERROR	
		{
		ShowAttyFinderButton = 0
		IniWrite, %ShowAttyFinderButton%, C:\Users\%Computername%\AHKPrefs.ini, Username, ShowAttyFinderButton
		IniRead,   ShowAttyFinderButton,  C:\Users\%Computername%\AHKPrefs.ini, Username, ShowAttyFinderButton
		}			
...
if (NewCPI)
	...
	IfWinActive, ahk_exe chrome.exe
		...
		oppNumber := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_ActionType').value").Value
		
		if instr(oppNumber, "Deadline to File Opposition")
			{
			oppNumber := StrReplace(oppNumber, "Deadline to File Opposition (", "")
			oppNumber := StrReplace(oppNumber, ")", "")
			oppRemarks = Possible Opposition against:\r\nApplication #: %oppNumber%\r\nMark: *\r\nApplicant: *\r\nClass: *\r\nOur Mark: *
			Page.Evaluate("document.getElementById('ctl00_Detail_Remarks').value = '" oppRemarks "'")
			}
		Page.Disconnect()
		}
	else
...
if (NewCPI) ; NEWCPI
	...
	NewValue := Trim(Clipboard) . "*"
	AllAsVariable := "document.getElementById('" Attribute "').value = '" NewValue "';"
	Page.Evaluate(AllAsVariable)
	Page.Evaluate("document.getElementById('ctl00_Detail_tplSearchButton_input').click();")
	Page.Disconnect()
	}
else	
	{
...
Gui, 1: Add, Tab3, x1 y0 w300 h390 +Center, Miscellaneous|Multiuse|Other Scripts|Keybindings| ;
Gui, 1: Tab, 1
...
Gui, 1:Font, s6
Gui, 1:Add, Text, x255 h10 y375 gCherSystemSwitch vCherSystemSwitch, v.10.25.2021   ; Version: change to 1.23.2017 when released
Gui, 1:Font 
...
Gui, 1:Add, Text, x10 y50 , Includes: Calendar Extender, Remarks Suggester,`n                           Record Opener 
...
Gui, 1:Add, Groupbox, cPurple x-3005 y25 w290 h66 +Center Section, HotCorners Shortcuts (To be removed.)
Gui, 1:Add, Edit, +ReadOnly  Sort R1    x-3008 ys+12 w90 h16  Disabled                   , Quick Settings   ;vHotCornerTL
Gui, 1:Add, ComboBox,        Sort R20   x-3008 ys+38 w90 h16  Disabled vHotCornerBL, 
Gui, 1:Add, ComboBox,        Sort R20 x-3201 ys+12 w90 h16  Disabled vHotCornerTR, 
Gui, 1:Add, ComboBox,        Sort R20 x-3201 ys+38 w90 h16  Disabled vHotCornerBR, 
Gui, 1:Add, Text, 		x-3100 ys+18 w98 h40 +Center, Hold Control and move mouse to far corners of screens.
...
Gui, 1: Tab, 3
Gui, 1:Add, Groupbox, cBlue x5 y25 w290 h105 +Center, Other Scripts
...
Gui, 1: Tab
Gui, 1:Add, Edit, ReadOnly h105 w300 x0 y470, Update Notes 10/12/2021: ***PLEASE READ!!!***`n`n-I made some changes. Restarting won't close Chrome anymore. If you get a scary error message, try closing all of your Chrome windows and open Chrome (AHK) from your desktop. You should add the correct docketing links as bookmarks so you can open them.`n`n-You can find updated scripts at: H:\Docketing\AutoHotKey\Scripts\New CPI Scripts. Right-click and send to Desktop as shortcut, or copy/paste where you want them. If you can't find one here, it is probably already magically updated.`n`n-I suggest you change your default browser to Chrome. (... menu in Chrome > Settings > Default Browser)`n`n-Added an AttyFinder hotkey if you don't want to use the new menu system. You can use it anywhere in CPI that you have a Client Code. The hotkey is Windows + A until I let you customize it.`n`n-The Settings Menu is a bit of a mess right now. I'll have to clean it up later.

global CherToggle = 0
Gui, 1: Tab, 3
Gui, 1: Add, Button, gRunDoubleTap 					+Center xp+12 y39 		w90 h42, DoubleTap
Gui, 1: Add, Button, gRunActionInjector 			+Center xp+93 yp    	w90 h42, F-Keys`nAction Injector
Gui, 1: Add, Button, gRunWoWbar 					+Center xp+93 yp 		w90 h42, WoWbar
Gui, 1: Add, Button, gRunCustomDocketShortcuts 		+Center xp-186 yp+43 	w90 h42, Custom`nDocketing Shortcuts
Gui, 1: Add, Button, gRunIndivFacingEmailer 		+Center xp+93 yp 		w90 h42, Individual Facing`nReport Emailer
Gui, 1: Add, Button, gPCT220						+Center xp+93 yp 		w90 h42, PCT 220`nDate Generator


Gui, 1: Add, Groupbox, cRed x-2005 yp+50 w290 h143 +Center, Experimental Scripts
Gui, 1: Add, Button, 		 		+Center xp+7 yp+15 	w276 h120, ▁ ▂ ▄ ▅ ▆ ▇ █ DocketMods █ ▇ ▆ ▅ ▄ ▂ ▁`n`n-HIGHLY experimental, but shouldn't break anything.`n-Updated pretty often, if I have any free time, that is.`n`nIncludes: Custom colors, built-in notepad,`nshortcut buttons, custom top bar, CID Mods, etc.

Gui, 1:Add, Groupbox, cRed x-2005 y-1780 w290 h43 +Center, To be determined...

Gui, 1: Tab
Gui, 1: Font, bold underline
Gui, 1:Add, Text, y392 x0 w301 +Center, Other Modifications
Gui, 1: Font,
Gui, 1:Add, Checkbox, Checked1 y412 x-2005 vModBar Checked0, Show Shortcuts Bar
Gui, 1:Add, Checkbox, Checked1 yp+20 xp vHideCPILogo Checked0, Hide CPI Logo
Gui, 1:Add, Checkbox, Checked1 yp+20 xp vShowPastDue Checked0, Highlight Past Due Dates
Gui, 1:Add, Checkbox, Checked1 y412 x-2155 vShowAttyFinder Checked0, Show AttyFinder Buttons
Gui, 1:Add, Checkbox, Checked1 yp+20 xp vShowStatusHighlight Checked0, Show Status Highlight
Gui, 1:Add, Checkbox, Checked1 yp+20 xp Disabled Checked0, TBD

Gui, 1: Font, bold underline
Gui, 1:Add, Text, y392 x0 w301 +Center, New CPI Docketing Options
Gui, 1: Font,
Gui, 1:Add, Text, yp+15 x5 w296 +Center, You can invoke this by pressing Control + Space.`nDepending on when you invoke it, you'll see different options.`nYou can close with the X or the use Control+ Space again.

Gui, 1:Add, Checkbox, Checked1 yp+45 x5 vShowAttyFinderButton Checked%ShowAttyFinderButton%, Show AttyFinder Button
Gui, 1:Add, Checkbox, Checked1 yp xp+150 vShowSuperEditOptions Checked%ShowSuperEditOptions%, Show SuperEdit Options
 

Gui, 1:Add, Radio, x-2075 y575 h25 w75 vNewCPI, Old CPI
Gui, 1:Add, Radio, xp+75 yp hp wp Checked1, New CPI

Gui, 1: Show, h576 w300, Docketing Scripts     ;h390 w300
...
RunActionInjector:
Run "\\docs-oc\files\Docketing\AutoHotKey\Scripts\New CPI Scripts\Other\Originals - DO NOT USE\SERIOUSLY DON'T USE THESE!\F-Keys Action Injector (New CPI).ahk"
return

RunWoWbar:
Run "\\docs-oc\files\Docketing\AutoHotKey\Scripts\New CPI Scripts\Other\Originals - DO NOT USE\SERIOUSLY DON'T USE THESE!\WoWbar (New CPI).ahk"
return

RunCustomDocketShortcuts:
Run "\\docs-oc\files\Docketing\AutoHotKey\Scripts\New CPI Scripts\Other\Originals - DO NOT USE\SERIOUSLY DON'T USE THESE!\Custom Docketing Shortcuts (New CPI).ahk"
return
...
;	GroupAdd, ChromeInstances, ahk_exe chrome.exe
;	GroupClose,  ChromeInstances, A

; Maybe something to do with ErrorLevel

	FileCreateShortcut, % "C:\Program Files (x86)\Google\Chrome\Application\chrome.exe", %A_Desktop%\Chrome (AHK).lnk, , % "--remote-debugging-port=9222 -- '%1'", % "Debug Mode version of Chrome so AHK can connect." ;,  IconFile, 
;	FileCreateShortcut, % "C:\Program Files\Google\Chrome\Application\chrome.exe", %A_Desktop%\Chrome (AHK) 64-bit.lnk, , % "--remote-debugging-port=9222 -- '%1'", % "Debug Mode version of Chrome so AHK can connect. (64-bit)" ;,  IconFile, 
...
;	Page.Call("Page.navigate", {"url" : "https://web05.computerpackages.com/Knobbe/" })
;	Page.Call("Target.createTarget", {"url" : "https://web05bak.computerpackages.com/KnobbeAgent/default.aspx" })
	Page.WaitForLoad()
	Page.Disconnect()
...
IniWrite, v.10.25.2021, \\docs-oc\files\Docketing\AutoHotKey\.ini Files - DO NOT TOUCH!\ImageSearch\%Computername%.ini, Version, Version
if CustomColor = Pink
	CustomColor = FF69B4
...
APIGetAttorneys(CPIClientCode, NewSystem)
{
value := ""
...
if (NewSystem == "Patent")
	EndPoint2 = SELECT t1.CaseNumber, t2.Attorney FROM tblPatInvention t1 INNER JOIN tblPatAttorneyInv t2 ON t2.InvId = t1.InvId WHERE t1.CaseNumber 
else
	EndPoint2 = SELECT t1.CaseNumber, t2.Attorney FROM tblTmkTrademark t1 INNER JOIN tblTmkAttorney t2 ON t2.TmkId = t1.TmkId WHERE t1.CaseNumber 
...
Loop % value.Table.length()
	attys.Push(SubStr(value.Table[A_Index].Attorney, 1, 3))
	
return % attys	
}


APIGetCountry(NewClientCode, NewSystem)
{
user := "Paperboy@knobbe.com"
pass := "knobbedocket"
...
if (NewSystem == "Patent")
	EndPoint2 := "SELECT country FROM tblPatCountryApplication WHERE casenumber "
else if (NewSystem == "Trademark")
	EndPoint2 := "SELECT country FROM tblTmkTrademark WHERE casenumber "
	
EndPoint3 := "= '" NewClientCode "';"
http := ComObjCreate("WinHttp.WinHttpRequest.5.1")
http.Open("GET", EndPoint1 . EndPoint2 . EndPoint3, true)
http.SetRequestHeader("Authorization", "Basic "  Base64Encode(user ":" pass) )
http.Send()
...
newLocation := Page.Evaluate("window.location.href").Value

if InStr(newLocation, "countryapplication")
	{
	CPIClientCode := Page.Evaluate("document.getElementById('ctl00_Detail_CaseNumber_Input').value").Value
	System := "Patent"
	}
else if InStr(newLocation, "tmkfrmActionDue")
	{
	CPIClientCode := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_CaseNumber').value").Value
	System := "Trademark"
	}
else if InStr(newLocation, "patfrmActionDue")
	{
	CPIClientCode := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_CaseNumber').value").Value
	System := "Patent"
	}	
else if InStr(newLocation, "Invention")
	{
	CPIClientCode := Page.Evaluate("document.getElementById('ctl00_Detail_CaseNumber').value").Value
	System := "Patent"	
	}
else if InStr(newLocation, "tmkfrmtrademark")
	{
	CPIClientCode := Page.Evaluate("document.getElementById('ctl00_Detail_CaseNumber').value").Value
	System := "Trademark"
	}	

CPICountry := APIGetCountry(CPIClientCode, System)

Attorneys := []
if InStr(newLocation, "ActionDue")
	{
	Attorneys := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_Attorneys').value;").Value
	Attorneys := StrReplace(Attorneys, " ", "")
	Attorneys := StrSplit(Attorneys, ",")
	}
else 
	{
	Attorneys := APIGetAttorneys(CPIClientCode, System)
	} 
...
CPIAttyFinder2(Attorneys, CPIClientCode, CPICountry)
return

CPIAttyFinder2(Attys, CPIClientCode, CPICountry)
{
;Page := ChromeInst.GetPage()
;newLocation := Page.Evaluate("window.location.href").Value
;Page.Disconnect()
...
if InStr(CPICountry, "US")
	{
	if (ATPartner == "-none-")
		NewAttyString := NewAttyString . ", *FORUS, " . CPIClientCode . FORUS
	else
		NewAttyString := NewAttyString . ", " ATPartner . ", *FORUS, " . CPIClientCode . FORUS	
	}
else if (CPICountry != "")
	{
	if (ATPartner == "-none-")
...
if InStr(Country, "US")
	{
	if ATPartner = -none-
		TempAttorneys := TempAttorneys . ", *FORUS, " . ClientCode . FORUS
	else
		TempAttorneys := TempAttorneys . ", " ATPartner . ", *FORUS, " . ClientCode . FORUS	
	}
else if Country != 
	{
	if ATPartner = -none-
...
ie := WBGet()
;if InStr(ie.LocationURL, "oc-docketing")
QuickDate()
return
...
Gui, Add, Button, x-2000 y35 w90 h25 gOpenWorkSpace, WorkSpace	
...
if (NewCPI) ; NEWCPI
	...
	WinActivate, ahk_exe chrome.exe
	Page := ChromeInst.GetPage()
	OutputVar := "https://web05bak.computerpackages.com/knobbeAgent/trademark/tmkschTrademark.aspx"
	; Page.Call("Target.createTarget", {"url" : OutputVar })
	Page.Call("Page.navigate", {"url" : OutputVar })
	Page.WaitForLoad()
	Attribute := "ctl00_Detail_CaseNumber_TextBox"
...
if (NewCPI) ; NEWCPI
	...
	WinActivate, ahk_exe chrome.exe
	Page := ChromeInst.GetPage()
	OutputVar := "https://web05bak.computerpackages.com/knobbeAgent/patent/patschcountryapplication.aspx"
	; Page.Call("Target.createTarget", {"url" : OutputVar })
	Page.Call("Page.navigate", {"url" : OutputVar })
	Page.WaitForLoad()
	Attribute := "ctl00_Detail_CaseNumber_TextBox"
...
if (NewCPI) ; NEWCPI
	...
	WinActivate, ahk_exe chrome.exe
	Page := ChromeInst.GetPage()
	OutputVar := "https://web05.computerpackages.com/knobbe/patent/patschInvention.aspx"
...
if (NewCPI) ; NEWCPI
	...
	WinActivate, ahk_exe chrome.exe
	Page := ChromeInst.GetPage()
	OutputVar := "https://web05bak.computerpackages.com/knobbeAgent/patent/patschInvention.aspx"
	; Page.Call("Target.createTarget", {"url" : OutputVar })
	Page.Call("Page.navigate", {"url" : OutputVar })
	Page.WaitForLoad()
	Attribute := "ctl00_Detail_CaseNumber_TextBox"
...
OpenWorkSpace:
MsgBox, this is broken right now.
...
qry := "SELECT * FROM dbo.vwAHKWorkspace WHERE WorkspaceName = '" Clipboard "'"
result := ADOSQL(qry)
FolderNumber := result[2,2] ; workspace name
...
if (FolderNumber == "")
	...
	{
	DocumentText := "KNOBBE-DMS.IMANAGE.WORK`n!nrtdms:0:!session:KNOBBE-DMS.IMANAGE.WORK:!database:KNOBBE:!folder:ordinary, " FolderNumber ":"
	Sleep 250
	; MsgBox, %FolderNumber% `n`n %DocumentText% `n`n %Computername% `n`n %Computername%
;	FileAppend, %DocumentText%, C:\Users\%Computername%\KNOBBE_%FolderNumber%.nrl
	FileAppend, %DocumentText%, %A_Desktop%\KNOBBE_%FolderNumber%.nrl
	Sleep 750
	WinActivate, ahk_exe OUTLOOK.EXE  ;  ahk_class rctrl_renwnd32
	Sleep 100
	; Run, C:\Users\%Computername%\KNOBBE_%FolderNumber%.nrl
	Run, %A_Desktop%\KNOBBE_%FolderNumber%.nrl
	MsgBox, wait
	Sleep 1500
	gosub RecordWorkSpaceOpener
	FileDelete, C:\Users\%Computername%\KNOBBE_%FolderNumber%.nrl
	return
	}
return
} 
...
Gui, Add, Checkbox, x221 y198 Checked%Option1% vOption1, 
Gui, Font, s8
...
NewClipboard = %Clipboard%
	if NewClipboard = ;
		...
		if InStr(Clipboard, "FA instructed to file divisional application by deadline of")
			{
			Page := ChromeInst.GetPage()
			BaseDate := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_BaseDate').value").Value
					
			year  := SubStr(BaseDate, 1,4)
			month := SubStr(BaseDate, 6,2) 
			day   := SubStr(BaseDate, 9,2)
			...
			if (SubStr(month, 1, 1) == "0")
				month := StrReplace(month, "0", "")

			if (SubStr(day, 1, 1) == "0")
				day := StrReplace(day, "0", "")
...
if (NewCPI)
	DocketingString = web05.computerpackages.com
else
...
if (NewCPI)
	{
	if WinExist("New CPI Docketing Options")
		{
		Gui, CPISuperEdit: Submit
		IniWrite, %RespAtty%, C:\Users\%Computername%\AHKPrefs.ini, Username, RespAtty
		Gui, CPISuperEdit: Destroy
...
Gui, CPISuperEdit: New, AlwaysOnTop ToolWindow, % "New CPI Docketing Options"

Gui, CPISuperEdit: Add, GroupBox, x3 y3 w294 h73, Shortcuts
Gui, CPISuperEdit: Add, Button, gCPISystemSwitch -Default w57  h50  xp+5 yp+16, System Switch
if (ShowAttyFinderButton)
	Gui, CPISuperEdit: Add, Button, gCPIAttyFinder -Default wp  h50  xp+58 yp, AttyFinder


Page := ChromeInst.GetPage()
newLocation := Page.Evaluate("window.location.href").Value

if (CanSeeCopy && InStr(newLocation, "ActionDue"))
	{
	Gui, CPISuperEdit: Add, GroupBox, x3 yp+58 w294 h73, Specialty Desk Functions
	Gui, CPISuperEdit: Add, Button, gCPIFindATAction	w57 h50 xp+5 yp+17, Find AT Action
	Gui, CPISuperEdit: Add, Button, gCPICloneActionToAT	wp+13  h50 xp+58 yp, Grab Data + Open New AT Action
	Gui, CPISuperEdit: Add, Button, gCPIGrabData		w45  h50 xp+76 yp, Grab Data Only
	Gui, CPISuperEdit: Add, Button, gCPIInjectData		w60  h50 xp+46 yp, Inject Data`n+`nRemarks
	Gui, CPISuperEdit: Add, Button, gCPIClearData		w45 h50 xp+61 yp, Clear Data
	}

if (ShowSuperEditOptions && InStr(newLocation, "ActionDue"))
	{
	Gui, CPISuperEdit: Add, GroupBox, x3 yp+58 w294 h155, SuperEdit
	Gui, CPISuperEdit: Add, Text,				w50 h20 xp+7 yp+17, Resp Atty:
	Gui, CPISuperEdit: Add, Edit,   vRespAtty	Center	w54 h20 xp+55 yp-3, % RespAtty

	Gui, CPISuperEdit: Add, Checkbox, vCPIAddRemarks Disabled	w70 xp+60 yp+4, Remarks?
	Gui, CPISuperEdit: Add, Checkbox, vCPIAutoExtend gCPIEnableAutoExtend  w100  xp+70 yp, Auto Extension?

	Gui, CPISuperEdit: Add, Button, gCPIExtend 	Section	w20  h20 x171 yp+18, 	% "1D"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		wp   hp  xp+20 yp, 		% "2D"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		wp   hp  xp+21 yp, 		% "3D"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		wp   hp  xp+20 yp,		% "4D"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		wp   hp  xp+21 yp,  	% "5D"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		wp   hp  xp+20 yp,  	% "6D"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		w40  h30  x171 yp+21, 	% "1W"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		wp   hp  xp+41 yp,  	% "2W"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		wp   hp  xp+41 yp,  	% "3W"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		wp   hp  x171  yp+31,	% "1M"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		wp   hp  xp+41 yp,  	% "2M"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		wp   hp  xp+41 yp,  	% "3M"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		w31  hp  x171  yp+31,  	% "4M"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		wp   hp  xp+30 yp,  	% "5M"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		wp   hp  xp+30 yp,  	% "6M"
	Gui, CPISuperEdit: Add, Button, gCPIExtend 		wp+1   hp  xp+30 yp,  	% "1Y"

	Gui, CPISuperEdit: Add, Button, gCPIQuickClose	w50  h56  x5 ys, Quick Close
	Gui, CPISuperEdit: Add, Button, Disabled	w50  hp  xp+51 yp, ???
	Gui, CPISuperEdit: Add, Button,	Disabled	w50  hp  x5 yp+57, ???
	Gui, CPISuperEdit: Add, Button, Disabled	w50  hp  xp+51 yp, ???
	}
	
if (CanSeeCopy && InStr(newLocation, "ActionDue"))
	{
	Gui, CPISuperEdit: Add, Edit, 	w165  h20 Center x300 y11, 
	Gui, CPISuperEdit: Add, Edit, 	w82  h20 Center xp yp+21, 
	Gui, CPISuperEdit: Add, Edit, 	wp  h20 Center xp+83 yp, 
	Gui, CPISuperEdit: Add, Edit, 	w165  h20 Center x300 yp+21, 
	Gui, CPISuperEdit: Add, Button, gCPIAdjustDate w20  h43 Center x300 yp+29, [1]
	Gui, CPISuperEdit: Add, Edit, 	% "hwndEDI1 w143 h20 Center xp+21 yp+1", 
	Gui, CPISuperEdit: Add, Edit, 	w71  h20 Center xp yp+21, 
	Gui, CPISuperEdit: Add, Edit, 	wp   h20 Center xp+72 yp, 

	Loop 4
		{
		Gui, CPISuperEdit: Add, Button, gCPIAdjustDate w20  h43 Center x300 yp+24, % "[" A_Index + 1 "]"
		Gui, CPISuperEdit: Add, Edit, % "hwndEDI" A_Index + 1 " w143 h20 Center xp+21 yp+1", 
		Gui, CPISuperEdit: Add, Edit, 	w71  h20 Center xp yp+21, 
		Gui, CPISuperEdit: Add, Edit, 	wp  h20 Center xp+72 yp,		
		}
	}
...
Gui, CPISuperEdit: Show, x%newposX% y%newposY% w300 ; h500
WinSet, Transparent, 255, New CPI Docketing Options
Page.Disconnect()
return
...
Page.Evaluate("var temp = document.documentElement.innerHTML.includes('Edit Due Date'); var temp2 = temp.toString();")
isOpen := Page.Evaluate("temp2").value

if (isOpen == "false")
	{
	if (A_GuiControl == "[1]")
		{
		Page.Evaluate("document.querySelectorAll('input[type=image][title=Edit]')[0].click()")
		Page.Disconnect()
		return
		}
	else if (A_GuiControl == "[2]")
		{
		Page.Evaluate("document.querySelectorAll('input[type=image][title=Edit]')[1].click()")
		Page.Disconnect()
		return
		}
	else if (A_GuiControl == "[3]")
		{
		Page.Evaluate("document.querySelectorAll('input[type=image][title=Edit]')[2].click()")
		Page.Disconnect()
		return
		}
	else if (A_GuiControl == "[4]")
		{
		Page.Evaluate("document.querySelectorAll('input[type=image][title=Edit]')[3].click()")
		Page.Disconnect()
		return
		}
	else if (A_GuiControl == "[5]")
		{
		Page.Evaluate("document.querySelectorAll('input[type=image][title=Edit]')[4].click()")
		Page.Disconnect()
		return
		}	
	}
...
if (A_GuiControl == "[1]")
	{
	GuiControlGet, CPIActionDue, , Edit6
	GuiControlGet, CPIDueDate, , Edit7
	GuiControlGet, CPIDateTaken, , Edit8
	}
else if (A_GuiControl == "[2]")
	{
	GuiControlGet, CPIActionDue, , Edit9
	GuiControlGet, CPIDueDate, , Edit10
	GuiControlGet, CPIDateTaken, , Edit11
	}
else if (A_GuiControl == "[3]")
	{
	GuiControlGet, CPIActionDue, , Edit12
	GuiControlGet, CPIDueDate, , Edit13
	GuiControlGet, CPIDateTaken, , Edit14
	}
else if (A_GuiControl == "[4]")
	{
	GuiControlGet, CPIActionDue, , Edit15
	GuiControlGet, CPIDueDate, , Edit16
	GuiControlGet, CPIDateTaken, , Edit17
	}
else if (A_GuiControl == "[5]")
	{
	GuiControlGet, CPIActionDue, , Edit18
	GuiControlGet, CPIDueDate, , Edit19
	GuiControlGet, CPIDateTaken, , Edit20
	}

Page := ChromeInst.GetPage()
Page.Evaluate("document.getElementsByClassName('DetailValue')[0].getElementsByTagName('INPUT')[0].value = '" CPIActionDue "'; document.getElementsByClassName('riTextBox')[0].value = '" CPIDueDate "'; document.getElementsByClassName('riTextBox')[1].value = '" CPIDateTaken "';")
WinActivate, ahk_exe chrome.exe
Page.Evaluate("document.getElementsByClassName('riTextBox')[0].focus(); document.getElementsByClassName('riTextBox')[0].blur(); document.getElementsByClassName('riTextBox')[1].focus(); document.getElementsByClassName('riTextBox')[1].blur()")
Page.Disconnect()
gosub CPIClickSave
Page.Disconnect()
return
...
client_code := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_CaseNumber').value").Value
action_type := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_ActionType').value").Value
action_base_date := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_BaseDate').value").Value

user := "Paperboy@knobbe.com"
pass := "knobbedocket"

if InStr(Page.Evaluate("window.location.href").Value, "patfrmActionDue")
	{
	EndPoint1 := "https://web05.computerpackages.com/knobbews/api/Knobbe?queryString="
	EndPoint2 = 
	(
	SELECT t3.ActionDue, t3.DueDate, t3.DateTaken %A_Space%
	...
	EndPoint3 := " t1.CaseNumber = '" client_code "' AND t2.ActionType = '" action_type "' AND t2.BaseDate = '" action_base_date "' ORDER BY t3.DueDate"
	}
else if InStr(Page.Evaluate("window.location.href").Value, "tmkfrmActionDue")
	{
	EndPoint1 := "https://web05.computerpackages.com/knobbews/api/Knobbe?queryString="
	EndPoint2 = 
	(
...
Loop % value.Table.length()
	{
	GuiControl, CPISuperEdit: , % "Edit" loop_offset, % value.Table[A_Index].ActionDue
	GuiControl, CPISuperEdit: , % "Edit" loop_offset + 1, % value.Table[A_Index].DueDate
	GuiControl, CPISuperEdit: , % "Edit" loop_offset + 2, % value.Table[A_Index].DateTaken
	loop_offset += 3
	SendMessage, 0xB1, -2, -1, , % "ahk_id" EDI%A_Index%  ; "0xB1", EM_SETSEL 
	}
...
Gui, CPISuperEdit: Show, NoActivate, Stored data cleared!

Gui, CPISuperEdit: Show, x%newposX% y%newposY% w450 ; h500
Sleep 35
Gui, CPISuperEdit: Show, x%newposX% y%newposY% w435 ; h500
Sleep 35
Gui, CPISuperEdit: Show, x%newposX% y%newposY% w420 ; h500
Sleep 35
Gui, CPISuperEdit: Show, x%newposX% y%newposY% w405 ; h500
Sleep 35
Gui, CPISuperEdit: Show, x%newposX% y%newposY% w390 ; h500
Sleep 35
Gui, CPISuperEdit: Show, x%newposX% y%newposY% w375 ; h500
Sleep 35
Gui, CPISuperEdit: Show, x%newposX% y%newposY% w360 ; h500
Sleep 35
Gui, CPISuperEdit: Show, x%newposX% y%newposY% w345 ; h500
Sleep 35
Gui, CPISuperEdit: Show, x%newposX% y%newposY% w330 ; h500
Sleep 35
Gui, CPISuperEdit: Show, x%newposX% y%newposY% w315 ; h500
Sleep 35
Gui, CPISuperEdit: Show, x%newposX% y%newposY% w300 ; h500
...
responseDate := ""
remarks := ""
GuiControl, CPISuperEdit: , Edit2, 
GuiControl, CPISuperEdit: , Edit3,
GuiControl, CPISuperEdit: , Edit4, 
GuiControl, CPISuperEdit: , Edit5, 
GuiControl, CPISuperEdit: , Edit6, 
GuiControl, CPISuperEdit: , Edit7, 
GuiControl, CPISuperEdit: , Edit8, 
GuiControl, CPISuperEdit: , Edit9, 
GuiControl, CPISuperEdit: , Edit10, 
GuiControl, CPISuperEdit: , Edit11, 
GuiControl, CPISuperEdit: , Edit12, 
GuiControl, CPISuperEdit: , Edit13, 
GuiControl, CPISuperEdit: , Edit14, 
GuiControl, CPISuperEdit: , Edit15, 
GuiControl, CPISuperEdit: , Edit16, 
GuiControl, CPISuperEdit: , Edit17, 
GuiControl, CPISuperEdit: , Edit18, 
GuiControl, CPISuperEdit: , Edit19, 
GuiControl, CPISuperEdit: , Edit20, 

Gui, CPISuperEdit: Show, x%newposX% y%newposY% w300 ; h500
SetTimer, UpdateTitle, -2000
return
...
Gui, CPISuperEdit: Show, NoActivate, Grabbing action data...
WinActivate, ahk_exe chrome.exe
Page := ChromeInst.GetPage()
...
Gui, CPISuperEdit: Show, NoActivate, Finding AppId or TmkId...
gosub CPIGrabData
Gui, CPISuperEdit: Show, NoActivate, Navigating to page...
...
UpdateTitle:
Gui, CPISuperEdit: Show, NoActivate, New CPI Docketing Options
return
...
GuiControl, CPISuperEdit: , Edit19, 
GuiControl, CPISuperEdit: , Edit20, 
Gui, CPISuperEdit: Show, , Grabbing action data...
WinActivate, ahk_exe chrome.exe
Page.Disconnect()
Page := ChromeInst.GetPage()
actionType := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_ActionType').value").Value
baseDate := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_BaseDate').value").Value
responseDate := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_ResponseDate').value").Value
global remarks := Page.Evaluate("document.getElementById('ctl00_Detail_Remarks').value").Value
GuiControl, CPISuperEdit: , Edit2, % actionType
GuiControl, CPISuperEdit: , Edit3, % baseDate
GuiControl, CPISuperEdit: , Edit4, % responseDate
GuiControl, CPISuperEdit: , Edit5, % remarks
Page.Disconnect()

gosub CPIGrabActionsDue
Gui, CPISuperEdit: Show, , Grabbing action data...
WinGetPos, mX, mY, mW, mH, A
newposX := mX ; + (mW / 2) - 150
newposY := mY ; + (mH / 2) - 175
...
Gui, CPISuperEdit: Show, x%newposX% y%newposY% w450 ; h500
Sleep 35
Gui, CPISuperEdit: Show, x%newposX% y%newposY% w470 ; h500
Page.Disconnect()
SetTimer, UpdateTitle, -2000
return

CPIInjectData:
Gui, CPISuperEdit: Show, NoActivate, Injecting action data...
Page := ChromeInst.GetPage()
Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_ActionType_Arrow').click();")
WinActivate, ahk_exe chrome.exe
Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_ActionType_Input').value = '" actionType "'; document.getElementById('ctl00_Detail_tplFormview_BaseDate_dateInput').value = '" baseDate "'; document.getElementById('ctl00_Detail_tplFormview_ResponseDate_dateInput').value = '" responseDate "';") 
...
Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_BaseDate_dateInput').focus(); document.getElementById('ctl00_Detail_tplFormview_ResponseDate_dateInput').focus();")

Page.Disconnect()
WinActivate, ahk_exe chrome.exe
Gui, CPISuperEdit: Show, NoActivate, And we're done!
SetTimer, UpdateTitle, -2000
gosub CPIInjectRemarks
return
...
Sleep 500
Page.Evaluate("document.getElementById('ctl00_Detail_Remarks').focus();")
Sleep 1000
SendRaw, % remarks
Sleep 500
Page.Evaluate("document.getElementById('ctl00_Detail_InsertButton_input').click(); document.getElementById('ctl00_Detail_UpdateButton_input').click()")
Page.Disconnect()
return

CPIEnableAutoExtend:
Gui, CPISuperEdit: Submit, NoHide
if (CPIAutoExtend)
	{
	GuiControl, CPISuperEdit: , Edit1, EXT
	GuiControl, CPISuperEdit: Disable, Edit1
;	GuiControl, CPISuperEdit: Disable, Button9
	GuiControl, CPISuperEdit: , Button12, 1
	}
else
	{
	GuiControl, CPISuperEdit: , Edit1, 
	GuiControl, CPISuperEdit: Enable, Edit1
;	GuiControl, CPISuperEdit: Enable, Button9
	GuiControl, CPISuperEdit: , Button12, 0
	}
return
...
if InStr(Page.Evaluate("window.location.href").Value, "ActionDueAgent")
	MsgBox, Only works on LIVE actions.
else if InStr(Page.Evaluate("window.location.href").Value, "patfrmActionDue")
	{
	Page.Disconnect()	
	Page := ChromeInst.GetPage()
	actionType := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_ActionType').value").Value
	baseDate := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_BaseDate').value").Value
	clientCode := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_CaseNumber').value").Value
	FindEquivATActionPAT(clientCode, actionType, baseDate)
	}
else if InStr(Page.Evaluate("window.location.href").Value, "tmkfrmActionDue")
	{
	Page.Disconnect()	
	...
	baseDate := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_BaseDate').value").Value
	clientCode := Page.Evaluate("document.getElementById('ctl00_Detail_tplFormview_CaseNumber').value").Value
	FindEquivATActionTM(clientCode, actionType, baseDate)
	}
Page.Disconnect()
return
...
if (A_GuiControl == "1W")	 ; 1 week
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 7)
else if (A_GuiControl == "2W") ; 2 weeks
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 14)			
else if (A_GuiControl == "3W") ; 3 weeks
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 21)	
else if (A_GuiControl == "4W") ; 4 weeks
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 28)	
else if (A_GuiControl == "1M") ; 1 month
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 101)	
else if (A_GuiControl == "2M") ; 2 months
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 102)
else if (A_GuiControl == "3M") ; 3 months
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 103)
else if (A_GuiControl == "4M") ; 4 months
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 104)
else if (A_GuiControl == "5M") ; 5 months
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 105)
else if (A_GuiControl == "6M") ; 6 months
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 106)
else if (A_GuiControl == "1Y") ; 1 year
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 100)
else if (A_GuiControl == "1D") ; 1 day
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 1)
else if (A_GuiControl == "2D") ; 2 day
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 2)
else if (A_GuiControl == "3D") ; 3 day
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 3)
else if (A_GuiControl == "4D") ; 4 day
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 4)
else if (A_GuiControl == "5D") ; 5 day
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 5)
else if (A_GuiControl == "6D") ; 6 day
	newDate := ExtendDays(Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value").Value, 6)
	
Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value = '" newDate "';")
WinActivate, ahk_exe chrome.exe
Page.Evaluate("document.getElementsByClassName('riTextBox')[0].focus(); document.getElementsByClassName('riTextBox')[0].blur()")
Page.Disconnect()
...
if (Page.Evaluate("document.getElementsByClassName('rcbInput').length").Value == 4)
	{
	Page.Evaluate("document.getElementsByClassName('riTextBox')[1].value = '" Today "';")
	WinActivate, ahk_exe chrome.exe
	Page.Evaluate("document.getElementsByClassName('riTextBox')[1].focus(); document.getElementsByClassName('riTextBox')[1].blur()")
	Page.Disconnect()
	if (Trim(RespAtty) != "")
		{
		gosub CPIInsertAtty
	...
	{
	Page.Evaluate("document.getElementsByClassName('riTextBox')[0].value = '" Today "';")
	WinActivate, ahk_exe chrome.exe
	Page.Evaluate("document.getElementsByClassName('riTextBox')[0].focus(); document.getElementsByClassName('riTextBox')[0].blur()")	
	Page.Disconnect()
	if (Trim(RespAtty) != "")
...
CPIAddRemarks:
Page := ChromeInst.GetPage()
CPINewRemarks := "Here are some new remarks."
Page.Evaluate("document.getElementsByTagName('TEXTAREA')[0].value = '" CPINewRemarks "\r\n' + document.getElementsByTagName('TEXTAREA')[0].value;")
Sleep 100
Page.Disconnect()
return
...
CPIClickSave:
Page := ChromeInst.GetPage()
Page.Evaluate("document.querySelectorAll('input[type=button][value=Save]')[0].click();")
Page.Disconnect()
return	

CPIInsertAtty:
RespAtty := Trim(RespAtty)
Page := ChromeInst.GetPage()
attorney := RespAtty
if (Page.Evaluate("document.getElementsByClassName('rcbInput').length").Value == 4)
	{
	Page.Evaluate("document.getElementsByClassName('rcbInput')[1].focus()")
	Sleep 100
	Page.Evaluate("document.getElementsByClassName('rcbInput')[1].value = '" attorney "';")
	Sleep 100
	SendInput, {Shift down}{Tab}{Shift up}
	}
else
	{
	Page.Evaluate("document.getElementsByClassName('rcbInput')[0].focus()")
	Sleep 100
	Page.Evaluate("document.getElementsByClassName('rcbInput')[0].value = '" attorney "';")
	Sleep 100
	SendInput, {Shift down}{Tab}{Shift up}
	}
...
FindEquivATActionPAT(code, action, base) {
user := "Paperboy@knobbe.com"
pass := "knobbedocket"

Gui, CPISuperEdit: Show, NoActivate, Grabbing Action Type and Base Date...

if InStr(action, "161")
	action := "INT-PAT EP RULE 161/162"

EndPoint1 := "https://web05.computerpackages.com/knobbews/api/Knobbe?queryString="
EndPoint2 := "SELECT appId, actId, CaseNumber, ActionType, BaseDate  FROM tblPatActionDueAgent WHERE casenumber "
EndPoint3 := "= '" code "' AND BaseDate = '" base "' AND ActionType LIKE '%" action "%';"

http := ComObjCreate("WinHttp.WinHttpRequest.5.1")
http.Open("GET", EndPoint1 . EndPoint2 . EndPoint3, true)
http.SetRequestHeader("Authorization", "Basic "  Base64Encode(user ":" pass) )
http.Send()
Gui, CPISuperEdit: Show, NoActivate, Looking for equivalent AT action...
while (http.Responsetext == "")
	Sleep 100
value := JSON.Load(http.Responsetext)

if (value.Table[1].actid != "")
	{
	Gui, CPISuperEdit: Show, NoActivate, Found equivalent AT action! Opening now...
	Page := ChromeInst.GetPage()
	Page.Call("Target.createTarget", {"url" : "https://web05bak.computerpackages.com/knobbeAgent/patent/patfrmActionDue.aspx?key=" value.Table[1].actid "&appId=" value.Table[1].appid })
	}
else	
	{
	Gui, CPISuperEdit: Show, NoActivate, Didn't find equivalent AT action :(.
	}	
...
FindEquivATActionTM(code, action, base) {
user := "Paperboy@knobbe.com"
pass := "knobbedocket"

EndPoint1 := "https://web05.computerpackages.com/knobbews/api/Knobbe?queryString="
EndPoint2 := "SELECT tmkId, actId, CaseNumber, ActionType, BaseDate  FROM tblTmkActionDueAgent WHERE casenumber "
EndPoint3 := "= '" code "' AND ActionType = '" action "' AND BaseDate = '" base "';"
http := ComObjCreate("WinHttp.WinHttpRequest.5.1")
http.Open("GET", EndPoint1 . EndPoint2 . EndPoint3, true)
http.SetRequestHeader("Authorization", "Basic "  Base64Encode(user ":" pass) )
http.Send()
...
if (value.Table[1].actid != "")
	...
	Gui, CPISuperEdit: Show, NoActivate, Found equivalent AT action! Opening now...
	Page := ChromeInst.GetPage()
	Page.Call("Target.createTarget", {"url" :"https://web05bak.computerpackages.com/knobbeAgent/trademark/tmkfrmActionDue.aspx?key=" value.Table[1].actid "&tmkId=" value.Table[1].tmkid })
	}
else	
	{
	Gui, CPISuperEdit: Show, NoActivate, Didn't find equivalent AT action :(.
