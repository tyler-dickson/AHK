...
IniRead, WindowPosX, C:\Users\%Computername%\AHKPrefs.ini, Username, WindowPosX
	if ((WindowPosX = ERROR) || (WindowPosX = ))
		{
		WindowPosX = 0
...
IniRead, WindowPosY, C:\Users\%Computername%\AHKPrefs.ini, Username, WindowPosY
	if ((WindowPosY = ERROR) || (WindowPosX = ))	
		{
		WindowPosY = 0
...
global SettingsY
global SidebarX
IniRead, CloseMsgForever, C:\Users\%Computername%\AHKPrefs.ini, CloseMsgBox, 05052017 
	if (CloseMsgForever = "ERROR") || (CloseMsgForever = 0)
		MsgBox, 52, WorkSpace Opener BETA and EFS Fill/EFS Fee Shortcuts, I have added the WorkSpace Opener BETA that was discussed during the US Agent meeting.`n`nAnthony B. also suggested having shortcuts to EFS Fill and EFS Fee Login since they are used so frequently (great idea!).`n`n-There is a new option on the EFS/HotCorners tab for WorkSpace Opener (BETA). Clicking the Start Script button will open the script, which will include instructions on how to use it and how it functions.`nI may move this option to another tab in the future.`n`n-EFS Fill can now be accessed with Windows + E (E as in "EFS").`n-EFS Fee Login can now be accessed with Windows + F (F as in "Fees").`n-You can also now map these functions to HotCorners as well.`n-I also added more prompts to the EFS Fill and EFS Fee Login to let you know they are actually working. (R.I.P. Bon Jovi reference... you'll be missed!)`n`nYou may need to change your custom keybindings if these happen to overlap with your custom choices.`n`nClick YES to hide this message for forever.`nClick NO to see this the next time you open the script.
		IfMsgBox Yes
			{
			IniWrite, 1, C:\Users\%Computername%\AHKPrefs.ini, CloseMsgBox, 05052017 
			goto Reopen
			}
		IfMsgBox No
			{
...
Clipwait .25
;Gui, Submit
TrayTip, Opening Record, Opening record in docketing.
...
Gui, 1:Font, s6
Gui, 1:Add, Text, x255 h10 y375, v.5.5.2017   ; change to 1.23.2017 when released
Gui, 1:Font
...
Gui, 1:Add, Groupbox, cFuchsia x5 y304 w290 h45 +Center, WorkSpace Opener (BETA)
Gui, 1:Add, Button, x10 y313 w70 h32 gRunWorkspaceOpener, Start Script
Gui, 1:Add, Text, Center x87 y319 h15, Click here to start the WorkSpace Opener.`nInstructions will show once it is started.
...
$^v::
;MsgBox, %AttyFinderJustUsed%, %AttyFinderJustUsed%
if AttyFinderJustUsed = 0
	{
	SendInput {Ctrl down}v{Ctrl up}
	AttyFinderJustUsed = 0
	return
	}
else if AttyFinderJustUsed = 1
	{
	SendInput {Ctrl down}v{Ctrl up}
	Sleep 500
	SendInput {Alt down}k{Alt up}
	AttyFinderJustUsed = 0
	return
	}
	AttyFinderJustUsed = 0
...
RunWorkspaceOpener:
Run "H:\Docketing\AutoHotKey\Scripts\WorkSpace Opener Beta.ahk"
return
...
IniWrite, v.5.5.2017, H:\Docketing\AutoHotKey\.ini Files - DO NOT TOUCH!\ImageSearch\%Computername%.ini, Version, Version
if CustomColor = Pink
	CustomColor = FF69B4
