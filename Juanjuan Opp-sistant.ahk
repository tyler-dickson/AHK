...
Gui, Startup: New, , Possible Opp-sistant
Gui, Startup: Add, Text,  Center w300, What kind of oppositions are you processing?
Gui, Startup: Add, Button, gWhichType vIncoming		w300, Incoming`n(sent directly from Juanjuan)
Gui, Startup: Add, Button, gWhichType vOutgoing 	w300, Outgoing`n(our redacted list sent to Juanjuan)
Gui, Startup: Add, Button, gWhichType vSpreadsheet 	w300, Spreadsheet`n(will set columns in next step)
Gui, Startup: Show
return

WhichType:
Gui, Submit
OppType := A_GuiControl
return
...
if OppType = Incoming
	...
	StringSplit, OppPieces, Clipboard, %A_Tab%
	StringReplace, OppPieces1, OppPieces1, App,, 
	NewPieces := StrSplit(OppPieces1, "`r`n")
	SplitGoods := StrSplit(OppPieces2, "`r`n")
	
	if !InStr(NewPieces[2], "Original Trademark Name")
		NewPieces.InsertAt(3, "")
		
	NewPieces[4] := Trim(NewPieces[4])
	
	Loop % SplitGoods.Length()
		{
		if InStr(SplitGoods[A_Index], "Goods and Services Computerised Translation")
			{
			Goods := SplitGoods[A_Index+1]
			break
			}
		}

;	MsgBox, % "Mark: " NewPieces[1] "`nApp No: " NewPieces[4] "`nDeadline: " OppPieces3 "`nCountry: " NewPieces[5] "`nApplicant: " NewPieces[6] "`nGoods: " Goods
	...
	while ie.busy or ie.ReadyState != 4 ;Wait for page to load
		Sleep, 100
	CurrentURL := ie.LocationURL ; grab current url
	NewAction := "Deadline to File Opposition (" NewPieces[4] ")"
	OppDeadline := OppPieces3
	ie.document.getElementByID("fldstrActionType_TextBox").Value := NewAction
	ie.document.getElementByID("flddteBaseDate").Value := OppDeadline
	ie.document.getElementByID("btnSave").Click()
	Sleep 1250
	NewRemarks := ATDate " Possible opposition against`nApplication Number: " NewPieces[4] " in Class " SplitGoods[1] "`n"" " NewPieces[1] " "" " "`nby " NewPieces[6] "`nOur Mark(s): HANBEV.204GEN`nDocketed per FLG. " Username "`n`nGoods: " Goods
	ie.document.getElementByID("btnEdit").Click()
	ie.document.getElementByID("fldstrRemarks").Value := NewRemarks
	Sleep 1000
	ie.document.getElementByID("btnSave").Click()
