...
Sleep 200
Gui, New, +AlwaysOnTop +Owner, Update Information
Gui, Add, Text, x50 y9 w200 h60 +Center, AT Partner Name is %ATPartner%. `nCurrent Username is %Username%. `nCurrent Remarks Date is %ATDate%.
Gui, Add, Text, x50 y60 w200 h80 +Center,Enter your AT Partner's Email Address:
Gui, Add, Edit, x50 y75 w200 h20 Limit +Center vATPartner, %ATPartner%
Gui, Add, Text, x50 y98 w200 h80 +Center,Enter your Username for Remarks:
Gui, Add, Edit, x50 y113 w200 h20 Limit +Center vUsername, %Username%
Gui, Add, Text, x50 y136 w200 h80 +Center,Enter the Date for Remarks:
Gui, Add, Edit, x50 y151 w200 h20 Limit +Center vATDate, %ATDate%
Gui, Add, Button, x62 y182 w80 h30 Default gaOK , Update
Gui, Add, Button, x162 y182 w80 h30 gCancel , Cancel
Gui, Add, Text, x22 , Control + CapsLock - Set Username/Date/AT Partner
Gui, Add, Checkbox, x10 Checked%Checkboxes2% vOption2, Shift + CapsLock - Quick Date and Initials
Gui, Add, Checkbox, x10 Checked%Checkboxes3% vOption3, Control + RightClick - Cal/Atty/Remarks in IE
Gui, Add, Checkbox, x30 Checked%Checkboxes1% vOption1, Auto-remarks on Calendar Extend?
Gui, Add, Checkbox, x10 Checked%Checkboxes4% vOption4, Control + `` [Left of 1] - System Switch Shortcut
Gui, Add, Checkbox, x10 Checked%Checkboxes5% vOption5, Windows + Space - System Switch Button (Optional/WIP)
Gui, Add, Checkbox, x10 Checked%Checkboxes6% vOption6, Control + 5/6 - Store/Paste Extra Clipboards
Gui, Add, Checkbox, x10 Checked%Checkboxes7% vOption7, Control + = - Create Shortcut Abbreviations
Gui, Add, Checkbox, x10 Checked%Checkboxes8% vOption8, Control + 1 - Open Hyperlink from Excel (Past Due, etc.)
if (xpos > 2150) & (ypos > 800)     
     {
     newypos := (ypos - 165)
     newxpos := (xpos - 230)
     Gui, Show, x%newxpos% y%newypos% h400 w300
     return 
     }
else if (ypos > 800)     
     {
     newypos := (ypos - 165)
     ;Msgbox, %newypos%
     Gui, Show, x%xpos% y%newypos% h400 w300
     return     
     }
else if (xpos > 2150)      
     {
     newxpos := (xpos - 230)
     ;Msgbox, %newxpos%
     Gui, Show, x%newxpos% y%ypos% h400 w300
     return     
     }
else
     {
     Gui, Show, x%xpos% y%ypos% h400 w300
	 return
     }
return
...
Gui, Add, Button, x330 y5 w100 h20 g2Cancel, Close
Gui, Show, ,Test
...
;	Gui, Submit
	...
	Sleep 100
	Gui, 2Gui:Destroy
	MsgBox, , You closed the System Switch button! ,You removed the button!`n`nNow you can't easily switch systems!`n`nPress Windows+Space to reopen it!
	Return
	}
