...
if InStr(Clipboard, "Comments")
	{
	MsgBox, Make sure you have the same number of rows as deadlines!
	Clipboard := StrReplace(Clipboard, "Comments", "Comments`n", 1)
	Clipboard := StrReplace(Clipboard, "`n`n", "`n")
	Clipboard := StrReplace(Clipboard, "`r`n", "`n")
	Clipboard := StrReplace(Clipboard, "This mark consists", "`n-----This mark consists")
	Clipboard := StrReplace(Clipboard, "This mark contains", "`n-----This mark consists of")
	Clipboard := StrReplace(Clipboard, "This mark is the same as the above mark.",  "-----This mark is the same as the above mark.")
	Clipboard := StrReplace(Clipboard, "This mark is same as the above one.",  		"-----This mark is the same as the above mark.")
	Clipboard := StrReplace(Clipboard, "This mark is same as the above mark.", 		"-----This mark is the same as the above mark.")
	tempArray := StrSplit(Clipboard, "`n")
	...
	Loop % tempArray.Length()
		{
		if InStr(tempArray[A_Index], "This mark is the same as the above mark")
			{
			tempArray[A_Index] := storedTemp
		
			}
		
		if InStr(tempArray[A_Index], "-----")
			{
			Clipboard := Clipboard "`n" tempArray[A_Index]
			storedTemp := tempArray[A_Index]
			}
		}
	Clipboard := StrReplace(Clipboard, "-----", "")
	TrayTip, Marks copied., Marks copied.
	
;	09/17/2021 - Possible opposition against:
;	Application number: *
;	Mark: *
;	Applicant: *
;	Class: *
;	Our Mark(s): HANBEV.204GEN
;	Rcvd via instr. *
	
	
	}	
else if InStr(Clipboard, "Mark/App No")
	{
	Clipboard := StrReplace(Clipboard, "`r`n", "X")
	Clipboard := StrReplace(Clipboard, " ", "X")
	Clipboard := StrReplace(Clipboard, "XXX", "`n")
	Clipboard := StrReplace(Clipboard, "X", "")
	Clipboard := StrReplace(Clipboard, "App", "")	
	Clipboard := StrReplace(Clipboard, "Mark/No", "App No")	
	TrayTip, App #s copied., App #s copied.
	}
else if InStr(Clipboard, "Opposition Deadline")
	{
	Clipboard := StrReplace(Clipboard, "`r`n", "`n")
	Clipboard := StrReplace(Clipboard, "`n`n", "`n")	
	TrayTip, Deadlines copied., Deadlines copied.
	}
else if InStr(Clipboard, "Applicant")
	{
	Clipboard := StrReplace(Clipboard, "`r`n", "@")
	temp := StrSplit(Clipboard, "@")
	newTemp := ""
	Loop % temp.Length() - 1
		{
		if (A_Index == 1)
			newTemp := temp[A_Index]	
		else
			{
			if (temp[A_Index] == "")
				temp[A_Index] := temp[A_Index - 1]
			if (temp[A_Index + 1] == "(individual)")
				{
				temp[A_Index] := temp[A_Index] " " temp[A_Index + 1]
				temp[A_Index + 1] := temp[A_Index]
				}
			else
				newTemp := newTemp "`n" temp[A_Index]	
			}
		}
	TrayTip, Applicants copied., Applicants copied.
	Clipboard := newTemp
	}
else if InStr(Clipboard, "Class(es)/Goods")
	{
	temp := StrSplit(Clipboard, "`r`n")
	classes := "Class"
	
	Loop % temp.Length()
		{
		if (temp[A_Index + 1] == "Subclass Description:")
			classes := classes "`n" temp[A_Index]

		}
	TrayTip, Classes copied., Classes copied.
	Clipboard := classes
	}
