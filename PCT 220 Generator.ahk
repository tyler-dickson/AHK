...
if PriorityMonth = 01
	...
	Priority16Day  := PriorityDay
	Priority30Day  := PriorityDay	
		if (PriorityDay = 29) || (PriorityDay = 30)
			Priority22Day  = 28
	...
	Priority22Year := PriorityYear + 2
	Priority30Year := PriorityYear + 3
		if ((PriorityDay = 29 ) || (PriorityDay = 30 ) || (PriorityDay = 31 )) && ((Priority16Year = 2020) || (Priority16Year = 2024) || (Priority16Year = 2028)) 
			Priority16Day = 29
		if PriorityDay = 31
			{
			Priority30Day = 30			
	...
	if (MailingDate2 > PriorityDate16)
		...
		GuiControl, ,Edit10, %MailingDate2Readable%
			if (MailingDate2 < TodaysDate)
				...
				GuiControl, Font, Edit10,
				Gui, Font
				MsgBox, , Dates Generating in the Past!, It looks like some of your dates might be generating in the past.`n`nConsider asking the Prosecution Team if the past due dates should be docketed.
				}
		}
...
if (PriorityDate30Numbers < MailingDateNumbers)
	{
	Gui, Font
	Gui, Font, cRed
	GuiControl, Font, Edit5,
	...
	Gui, Font
	ToolTip, More than 30M from Earliest Priority!, More than 30M from the earliest priority has passed!`n`nYou should only docket INT-PAT FOREIGN ACTION REFS (assuming a PCT 203 wasn't issued), and then confirm with the Prosecution Team if any other dates should be docketed!`n`nOpen the PCT 220 Checklist for further details?
