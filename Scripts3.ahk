...
Gui, Add, Checkbox, x10 vOption2, Shift + CapsLock - Quick Date and Initials
Gui, Add, Checkbox, x10 vOption3, Control + RightClick - Cal/Atty/Remarks in IE
Gui, Add, Checkbox, x10 vOption4, Control + `` [Left of 1] - System Switch Shortcut
Gui, Add, Checkbox, x10 vOption5, Windows + Space - System Switch Button (Optional/WIP)
Gui, Add, Checkbox, x10 vOption6, Control + 5/6 - Store/Paste Extra Clipboards
Gui, Add, Checkbox, x10 vOption7, Control + = - Create Shortcut Abbreviations
Gui, Add, Checkbox, x10 vOption8, Control + 1 - Open Hyperlink from Excel (Past Due, etc.)
if (xpos > 2150) & (ypos > 800)     
     {
     newypos := (ypos - 165)
     newxpos := (xpos - 230)
     Gui, Show, x%newxpos% y%newypos% h375 w300
     return 
     }
else if (ypos > 800)     
     {
     newypos := (ypos - 165)
     ;Msgbox, %newypos%
     Gui, Show, x%xpos% y%newypos% h375 w300
     return     
     }
else if (xpos > 2150)      
     {
     newxpos := (xpos - 230)
     ;Msgbox, %newxpos%
     Gui, Show, x%newxpos% y%ypos% h375 w300
     return     
     }
else
     {
     Gui, Show, x%xpos% y%ypos% h375 w300
	 return
     }
return
...
Gui, Add, DropDownList,x110 y5 w100 h20 R7 Sort gQCType vQC,|PCT|NPhase|Direct|Design|Trademark|Madrid|
