...
global SettingsY
global SidebarX
IniRead, CloseMsgForever, C:\Users\%Computername%\AHKPrefs.ini, CloseMsgBox, 03282017 
	if (CloseMsgForever = "ERROR") || (CloseMsgForever = 0)
		MsgBox, 20, Small update., Just some small changes in this update.`n`nYou shouldn't have any issues, but let me know if you encounter any weirdness.`n`n-EFS Cert Finder should be a bit faster and more reliable now.`n`n-Notifications will now appear as a bubble on the taskbar instead of in the middle of the screen. Let me know if you hate it and I can revert it.`n`n-I occasionally make small updates to the script throughout the day.`nIf you're using an outdated version, you'll receive a notice giving you the option to restart the script or defer until later.`n`nClick YES to hide this message for forever.`nClick NO to see this the next time you open the script.
		IfMsgBox Yes
			{
			IniWrite, 1, C:\Users\%Computername%\AHKPrefs.ini, CloseMsgBox, 03282017 
			goto Reopen
			}
		IfMsgBox No
			{
...
Gui, 1:Font, s6
Gui, 1:Add, Text, x255 h10 y375, v.3.28.2017   ; change to 1.23.2017 when released
Gui, 1:Font
...
IniWrite, v3.28.2017, H:\Docketing\AutoHotKey\.ini Files - DO NOT TOUCH!\ImageSearch\%Computername%.ini, Version, Version
if CustomColor = Pink
	CustomColor = FF69B4
