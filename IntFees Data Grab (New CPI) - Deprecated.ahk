...
Menu, Tray, Icon, mmcndmgr.dll , 50
...
Gui, New,  ToolWindow -AlwaysOnTop, IntFees Data Grab (New CPI)
Gui, Font, bold s14
...
Gui, Font, 
Gui, Font, bold s10
Gui, Add, Button, gNewCPIGrabPatentData x180 y195 w105 h35, Grab Data
Gui, Show, w465 h235
return
...
EndPoint2 = 
(
SELECT t2.CaseNumber, t2.Country, t2.SubCase, t2.ActionType, t3.ActionDue, t1.ApplicationStatus AS Status, t2.ResponseDate, t3.DueDate, t3.DateTaken, t3.Indicator, t2.AppId, t3.ActId %A_Space%
...
WHERE t1.Country NOT IN('US') AND t3.DateTaken IS NULL AND t3.ActionDue LIKE '%SearchCriteria%' AND %A_Space%
)
...
; MsgBox, % http.Responsetext
 MsgBox, % value.Table.length() " dates found! Injecting into Excel now!"
...
Loop % value.Table.length()
	...
	Clipboard := value.Table[A_Index].DueDate  "`t"		value.Table[A_Index].ActionDue "`t"		value.Table[A_Index].Indicator "`t" value.Table[A_Index].CaseNumber "`t" value.Table[A_Index].Status "`t" value.Table[A_Index].SubCase "`t" value.Table[A_Index].SubCase "`t" "`t"	value.Table[A_Index].ActId "`t"	value.Table[A_Index].AppId 
...
Gui, Show, NoActivate, IntFees Data Grab (New CPI)
return
