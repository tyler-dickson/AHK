...
Gui, New, ToolWindow AlwaysOnTop, Grab and Format Inventors
Gui, Add, Button, gGrabInventors w100 h100 x5 y5, Grab and Format Inventors
Gui, Show, w110 h110
return
...
inventors := []
ie := WBGet()
inventorTable := ie.document.getElementById("dgdInventors")

Loop % inventorTable.getElementsByTagName("TR").length - 1
	{
	temp := % inventorTable.getElementsByTagName("TR")[A_Index].innerText

	newInventor := StrSplit(temp, ",")
	temp := Trim(newInventor[2]) " " Trim(newInventor[1])
	if Clipboard =
		Clipboard := temp
	else
		Clipboard := Clipboard ", " temp

	}
TrayTip, Attorneys added to Clipboard, %Clipboard%
return
