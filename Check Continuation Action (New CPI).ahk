...
Gui, New, , Check Continuation Action (New CPI)
...
Gui, Add, ListView, x165 y55 w630 h400 Checked Right Grid vLoadedFiles , Client Code                            |Action                                                                            |Due Date        |Date Taken
...
Append:
Gui, Submit, NoHide
BackEnd := StrSplit(BackEnd, "`n") 
ListOfClientCodes := ""
Loop % BackEnd.Length()
	{
	if (ListOfClientCodes == "")
		ListOfClientCodes := "'" BackEnd[A_Index] "'"
	else 
		ListOfClientCodes := ListOfClientCodes  ","  "'" BackEnd[A_Index] "'"
	}
LV_Delete()
...
query := EndPoint1 EndPoint2 EndPoint3
...
Loop % BackEnd.Length() 
	{
	if InStr(http.Responsetext, BackEnd[OuterLoop])
		{
		Loop % value.Table.length()
			{
			if (BackEnd[OuterLoop] ==  value.Table[A_Index].CaseNumber)
				{
				LV_Add("", value.Table[A_Index].CaseNumber, value.Table[A_Index].ActionDue, value.Table[A_Index].DueDate, value.Table[A_Index].DateTaken)
				BackEnd[OuterLoop] := ""
				break
				}
			}
		...
		{
		LV_Add("", BackEnd[OuterLoop], "Action missing.", "---", "---")		
		}
	OuterLoop++
	}
http := ""
...
Loop % BackEnd.Length()
	...
	if InStr(http.Responsetext, BackEnd[OuterLoop])
		...
		Loop % value.Table.length()
			{
			if InStr(value.Table[A_Index].ActionDue, "File Continuing Application? Patent to Issue")
				{
				LV_Add("", value.Table[A_Index].CaseNumber, value.Table[A_Index].ActionDue, value.Table[A_Index].DueDate, value.Table[A_Index].DateTaken)
				;break
				}
		...
		{
		LV_Add("", BackEnd[OuterLoop], "Action not found.", "---","---" )
		}
	OuterLoop++
	}
*/
