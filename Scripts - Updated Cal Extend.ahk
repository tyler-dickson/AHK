...
global SidebarX
global RespAtty
IniRead, CloseMsgForever, C:\Users\%Computername%\AHKPrefs.ini, CloseMsgBox, 06022017 
	if (CloseMsgForever = "ERROR") || (CloseMsgForever = 0)
		MsgBox, 52, New and Improved Record Opener + EFS Fill, While researching another topic, I figured out how to improve the Record Opener, Quick Record Opener, and EFS Fill functions.`n`nTHESE WILL NOW ALL FUNCTION CORRECTLY IN YOUR CURRENT TAB!`n`nNo more having to worry about overwriting your first tab on accident, or any weirdness when using the EFS Fill function.`n`nJust a quick reminder that there is a new option on the EFS/HotCorners tab for WorkSpace Opener (BETA). Clicking the Start Script button will open the script, which will include instructions on how to use it and how it functions. Only a few of you have tried it, so give it a shot to see how it works, and if it would be useful.`n`nClick YES to hide this message for forever.`nClick NO to see this the next time you open the script.
		IfMsgBox Yes
			{
			IniWrite, 1, C:\Users\%Computername%\AHKPrefs.ini, CloseMsgBox, 06022017  
			goto Reopen
			}
		IfMsgBox No
			{
...
Gui, 1:Font, s6
Gui, 1:Add, Text, x255 h10 y375 gCherSystemSwitch vCherSystemSwitch, v.6.2.2017   ; change to 1.23.2017 when released
Gui, 1:Font
...
ClearA:
GuiControl, 1: , Button32, 1
GuiControl, 1: , Button33, 0
GuiControl, 1: , Button34, 0
return
...
IniWrite, v.6.2.2017, H:\Docketing\AutoHotKey\.ini Files - DO NOT TOUCH!\ImageSearch\%Computername%.ini, Version, Version
if CustomColor = Pink
	CustomColor = FF69B4
